...
def drop_old_duplicate_entries_from_table(migrate_engine, table_name,
    ...
    for row in migrate_engine.execute(duplicated_rows_select):
        #Do not remove row that has the biggest ID.
...
def _restore_indexes_on_deleted_columns(migrate_engine, table_name, indexes):
    ...
    #Restore indexes on `deleted` column
...
def _change_deleted_column_type_to_id_type_sqlite(migrate_engine, table_name,
                                                  **col_name_col_instance):
    #                 sqlaclhemy-migrate can't drop column with check
    ...
    def is_deleted_column_constraint(constraint):
        #                 There is no other way to check is CheckConstraint
    ...
    # Fix value of deleted column: False -> "" or 0.
    new_table.update().\
